...
## 2024-07-19 - [Fixing Sequential Database Queries]
**Learning:** Some controllers execute multiple independent database queries sequentially using `await` on each one in a row (e.g., `getKpi` in `DashboardController`). This unnecessarily slows down response times, turning total duration into the sum of each query's duration.
**Action:** Group independent queries using `Promise.all` so they execute concurrently, reducing the total duration to that of the slowest single query. Look for consecutive, un-linked `await db.select()...` or `await Model.findBy...` statements for easy concurrency wins.
