...
import { parseCreateUomInput, parseUpdateUomInput, parseUomListQuery } from "./uom.validation";
...
export class UomController {
  static async getAll(ctx: Context) {
    ...
    try {
      const parsed = parseUomListQuery(ctx.query);
      if (!parsed.success) {
        ctx.set.status = 400;
      ...
      const [totalRecord, records] = await Promise.all([
        UomModel.countAll({ searchTerm, filterColumn, isActive }),
        UomModel.findAll({ page, limit: internalLimit, orderBy, searchTerm, filterColumn, isActive }),
      ]);

      const totalPage = limit === 1000 ? 1 : Math.ceil(totalRecord / limit);
      const baseUrl = (process.env.APP_URL ?? "http://localhost:3000") + "/api/uoms";
  ...
  static async getById(ctx: Context) {
    ...
    try {
      ...
      return successResponse(correlationId, "Data found!", { record: uom });
    } catch (err: unknown) {
      ctx.set.status = 500;
      return failedResponse(correlationId, "Internal server error", 500, err instanceof Error ? err.message : "Unknown error");
    }
  ...
  static async create(ctx: Context & { user?: JwtPayload }) {
    const correlationId =
      (ctx.headers["x-correlation-id"] as string | undefined) ?? crypto.randomUUID();
    try {
      const parsed = parseCreateUomInput(ctx.body);
      if (!parsed.success) {
        ctx.set.status = 400;
        return failedResponse(correlationId, "Create data failed!", 400, parsed.error.issues[0]?.message ?? "Input invalid!");
      }

      const existing = await UomModel.findByCode(parsed.data.code);
      if (existing) {
        ctx.set.status = 400;
        return failedResponse(correlationId, "Create data failed!", 400, "UOM code already exists");
      }

      const uom = await UomModel.create(parsed.data, ctx.user?.sub);

      await logActivity({
        userId: ctx.user?.sub,
        module: MODULE_TYPE,
        action: "CREATE_DATA",
        description: `User ${ctx.user?.email} menambahkan data UOM "${uom.name}" dengan ID ${uom.id}`,
      });

      return successResponse(correlationId, "Data has been created", { record: uom });
    } catch (err: unknown) {
      ctx.set.status = 500;
      return failedResponse(correlationId, "Internal server error", 500, err instanceof Error ? err.message : "Unknown error");
    }
  ...
  static async update(ctx: Context & { user?: JwtPayload }) {
    ...
    try {
      ...
      if (parsed.data.code) {
        const codeExists = await UomModel.findByCode(parsed.data.code);
        if (codeExists && codeExists.id !== id) {
          ctx.set.status = 400;
          return failedResponse(correlationId, "Update data failed!", 400, "UOM code already exists");
        }
      }

      const updated = await UomModel.update(id, parsed.data, ctx.user?.sub);

      await logActivity({
        userId: ctx.user?.sub,
        module: MODULE_TYPE,
        action: "UPDATE_DATA",
        description: `User ${ctx.user?.email} mengubah data UOM ID ${id}`,
      });
  ...
  static async remove(ctx: Context & { user?: JwtPayload }) {
    ...
    try {
      ...
      await logActivity({
        ...
        module: MODULE_TYPE,
        action: "DELETE_DATA",
        description: `User ${ctx.user?.email} menghapus data UOM ID ${id}`,
      });
